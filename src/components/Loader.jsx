  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-md transition-colors duration-300">
      <div className="relative w-24 h-24 mb-10">
        <div className="absolute inset-0 rounded-full border-4 border-slate-200 dark:border-slate-700 shadow-inner"></div>
        ...
        <div className="absolute inset-3 rounded-full bg-gradient-to-tr from-primary-100 dark:from-primary-900/40 to-teal-50 dark:to-teal-900/20 animate-pulse flex items-center justify-center">
      ...
      <div className="h-12 flex flex-col items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="flex items-center gap-3 bg-white dark:bg-slate-800 px-6 py-3 rounded-2xl shadow-soft dark:shadow-black/20 border border-slate-100 dark:border-slate-700"
          >
            <div className="animate-pulse">{steps[currentStep].icon}</div>
            <span className="text-slate-700 dark:text-slate-200 font-medium">{steps[currentStep].text}</span>
          </motion.div>
        </AnimatePresence>
      </div>
      
      <div className="w-64 h-1.5 bg-slate-200 dark:bg-slate-700 mt-6 rounded-full overflow-hidden">
         <motion.div 
